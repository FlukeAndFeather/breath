---
title: "Blue whale breathing rates"
...
bw_prh <- readxl::read_excel("bw_prh_guide.xlsx") %>% 
  mutate(
...
35 deployments with NetCDF

```{r dives}
# Deployment metadata
this_row <- filter(bw_prh, !is.na(prh_path)) %>% slice(1)
this_id <- this_row$id
...
plot_dive_event(this_dives, 25, 60)
```
...
# something's wrong with bw180827-46 and bw180905-53
med_rates <- breath_rates %>% 
  ...
  arrange(med_rate)
breath_rates %>% 
  filter(!id %in% c("bw180827-46", "bw180905-53")) %>% 
